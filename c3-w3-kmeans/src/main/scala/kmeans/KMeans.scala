...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    var map = points.groupBy { case point => findClosest(point, means) }
    if( map.isEmpty ) {
      means.foreach { m => map += (m -> IndexedSeq()) }
    }
    map
  }
